...
printf("ARE YOU READY????\n"
 "program is loading!!!!!!! PLEASE WAIT FOR FEW SECONDS\n");
 sleep(10);
printf("@SAIKAT\n");
...
void file_reading(s2 file[],int* linecount){
	FILE *p;
	p=fopen("test program1.cpp","r");// GIVE YOUR PATH IN THE <<<<< test program1.cpp >>>>>>>>>. That will read another C program   
	int i=0,k=0;
    while(!feof(p)){
...
/*puts*/	if(file[i].l[k]=='p'&&file[i].l[k+1]=='u'&&file[i].l[k+2]=='t'&&file[i].l[k+3]=='s'&&file[i].l[k+4]=='('){*pts+=1;}
	...
	if(file[i].l[k]=='v'&&file[i].l[k+1]=='o'&&file[i].l[k+2]=='i'&&file[i].l[k+3]=='d'&&file[i].l[k+4]==' '){ // checking void
